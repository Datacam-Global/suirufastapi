...
class TwitterHateSpeechModel:
    """Wrapper for the Twitter hate speech model and vectorizer"""
    def __init__(self, base_dir):
        self.model = joblib.load(os.path.join(base_dir, "twitter_hate_model.joblib"))
        self.vectorizer = joblib.load(os.path.join(base_dir, "twitter_hate_vectorizer.joblib"))
...
class CameroonHateSpeechDetector:
    ...
    def __init__(self, use_twitter_model=True):
        ...
        self.twitter_model = None
        if use_twitter_model:
            try:
                base_dir = os.path.dirname(os.path.abspath(__file__))
                self.twitter_model = TwitterHateSpeechModel(base_dir)
            except Exception as e:
                logger.warning(f"Twitter hate speech model could not be loaded: {e}")
    ...
    def detect_hate_speech(self, text: str, model: str = "auto") -> HateSpeechResult:
        ...
        model: "auto" (default) uses both, "cameroon" for original, "twitter" for Twitter model only
        """
        ...
        if model == "twitter" and self.twitter_model:
            result = self.twitter_model.predict(text)
            return HateSpeechResult(
                text=text,
                ...
                severity="unknown",
                timestamp=datetime.now(),
                explanation="Predicted by Twitter hate speech model."
            )
        elif model == "cameroon" or not self.twitter_model:
            ...
            # Use both models, return hate if either detects hate
            cameroon_result = self.detect_hate_speech(text, model="cameroon")
            twitter_result = self.detect_hate_speech(text, model="twitter")
            is_hate = cameroon_result.is_hate_speech or twitter_result.is_hate_speech
            confidence = max(cameroon_result.confidence, twitter_result.confidence)
            explanation = f"Cameroon: {cameroon_result.explanation}; Twitter: {twitter_result.explanation}"
            return HateSpeechResult(
                text=text,
                is_hate_speech=is_hate,
                confidence=confidence,
                detected_keywords=cameroon_result.detected_keywords,
                category=cameroon_result.category if cameroon_result.is_hate_speech else twitter_result.category,
                severity=cameroon_result.severity if cameroon_result.is_hate_speech else twitter_result.severity,
                timestamp=datetime.now(),
                explanation=explanation
            )
    ...
    def predict(self, text: str) -> Tuple[str, float, str]:
        ...
        if openai_api_key:
            try:
                ...
                print(f"[OpenAI Request] url={url} data={data}", file=sys.stderr)
                print(f"[OpenAI Response] status={response.status_code} body={response.text}", file=sys.stderr)
                ...
                return "unverified", 0.0, "unknown"
        print("[OpenAI] Missing configuration or failed to execute.", file=sys.stderr)
        return "unverified", 0.0, "unknown"
